#ifndef PROTOCOL_MODBUS_H
#define PROTOCOL_MODBUS_H

#include "protocolbase.h"

/**
 * @brief ModBus-RTU协议实现
 */
class ProtocolModBus : public ProtocolBase
{
public:
    // 基本信息
    QString name() const override { return "ModBus-RTU"; }
    QString displayName() const override { return "ModBus-RTU (旋转电爪)"; }
    QString description() const override { return "ModBus-RTU 旋转电爪控制协议"; }
    
    // 帧结构定义
    QList<ProtocolField> frameFields() const override;
    QList<ProtocolField> parseFields() const override;
    
    // 数据处理
    QString buildFrame(const QStringList &values) const override;
    QStringList parseFrame(const QString &frame) const override;
    QString calculateChecksum(const QString &data) const override;
    
    // 验证
    bool validateFrame(const QString &frame) const override;
    QString getFieldDescription(const QString &fieldName, const QString &value) const override;

private:
    QStringList getAvailableFunctions() const;
    QString getFunctionDescription(const QString &function) const;
    QString getAddressDescription(const QString &address) const;
    QString extracted(const QString &address, const QString &data) const;
    QString getDataDescription(const QString &address,
                               const QString &data) const;
};

#endif // PROTOCOL_MODBUS_H
